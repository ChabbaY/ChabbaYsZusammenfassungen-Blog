...
- instead of states, descriptions of states like feature vectors are used (**continuous State Space**)
  ...
  - methods:
    - **Value function approximation**: learning a function that describes the value function v(S) or state-value function q(S,A)
      - learnt value function & implicit policy (e.g. $\epsilon$-greedy)
    ...
    - **Policy gradient** methods: directly learn a function to predict the best action
      ...
      - Pro: better convergence, effective at high dimensions & continuous action space
      - Con: often only local optimum found, high variance
      - allows for stochastic policies
        - even better convergence possible
    - **Actor Critic** methods:
      - learnt value function & learnt policy
      - **actor**: learns stochastic policy based on critic's estimates
      - **critic**: learns action-value function to predict future rewards
      - following approximate policy gradient

## XI. Multi-Agent Settings

- using knowledge about other agents to make better decisions
- Rewards
  - **zero-sum**
  - **shared rewards**
  - **individual rewards**

### A. Antagonistic Settings

- agents compete for rewards (negative correlation)
- assuming that opponent prefers to maximize their rewards (will usually follow optimal policy)
- **Nash Equilibrium**: where each opponent cannot increase their expected reward by changing their policy but changing both might lead to a better expected reward for both
  - **Prisoner dilemma**: two robbers are offered a deal if they testify; both testifying will lead to 5 years each, not as good as the joint strategy to both refuse and get 1 year each
- **Antagonistic Search**:
  - tree with all possible outcomes
  - propagation from leave to root, where the player maximizes their rewards and the opponent minimizes the rewards
  - generally a search until the end is not feasible, heuristics / estimations are used instead
  - **Min-Max Search** with **Alpha-Beta Pruning**:
    - DFS with iterative deepening
    - $\alpha$ is the guaranteed min. reward on this sub-tree
    - $\beta$ is the max. reward the opponent will allow
    - $\beta < \alpha$ indicates the opponent would allow less reward than could be achieved taking the other branch
- **Monte Carlo Tree Search**:
  - when high branching factor or heuristics hard to determine
  - tree is sampled based on Monte Carlo Learning of simulated playouts
  - with **Upper Confidence Bound**: select path with highest win rate & rollout episode on a basis heuristic, then use backpropagation to update win rates
  - also applicable for antagonistic search but not limited to
  - e.g. used in Alpha Go

### B. Collaborative Settings

- possible collaborations:
  - **joint team reward**: no single assignment possible
  - **shared observation**: better estimation of belief states
    - communication become expensive & not all observations are equally important to all agents
      - joins based on spatial overlap
      - using central peer to calculate belief states & redistribute them to agents
  - **shared policy**: knowing what teammates will do
    - **Centralized Training, Decentralized Execution** (CTDE)
      - **COMA** (Counterfactual Multi-Agent Policy): isolate individual contributions by fixing all other agents' actions and evaluate effect of different actions
      - **QMIX**: joint action-value function is learnt and then decomposed by **Value Decomposition Networks** (VDN)
      - **QTRAN**: guarantees **Individual-Global-Max** (IGM), when an agent's best action depends on a teammate's action
  - **shared experience**: learning faster
    - requires off-policy learning (experience not generated by same policy)
    - assumes same rewards for all agents
    - shared information is incomplete
  - **passive collaboration**: correlation in reward functions
    - no active form of sharing information

### C. Imitation Learning

- samples of good behavior (experience, known reward) or just samples (demonstration, reward not known but success expected based on player's reputation) are provided
- **Behavioural Cloning**: learn expert's policy based on sufficient sample episodes
  - overfits policy to observations
  - no long-term planning
  - simple & efficient
- **Inverse Reinforcement Learning**:
  - when too few samples & no interactive expert
  - learns reward function instead of policy, policy can be derived from reward function
- applications:
  - make a player behave like a real one
  - learn policies for hard opponents & analyze weaknesses
  - agent learns from human experts
  - learn own policy & find out where it deviates from the optimal
...
- fraud influences a MMO's long term success
  - players loose interest
  - providers may lose money (micro transactions)
- reasons for fraud:
  - **economic**: trading game goods
  - **saving time**: tasks with low skill requirement in player absence
  - **prestige**: success coupled with reputation
  - **fun**: beating other players is fun
- ways to cheat:
  - **client mods**: getting more information, bypassing physics, etc.
  - **modifying** other **system components**: manipulating network, device drivers, etc.
  - **botting**: saving time (farm bots) or increasing power (poker bots, aim bots)
  - **macros**, scripts: partly control/optimize user input
  - **account kidnapping**: takeover of another account (e.g. selling their goods)
  - **illicit trade** with virtual goods: on external platform
- unsuitable behavior:
  - early leaving
  - flaming
  - passiveness
...
- **Heat Map**:
  - displaying events as 2D-density distribution
  - grid position & size influence result
  - smooth curves with **Kernel density estimator**
    - **Gauss-kernel**
    - Cauchy-kernel
    - Picard-kernel
    - **Epanechnikow-kernel**: only defined in [-1; 1]
- **Subsequences** & **Partitioning**
  - finding typical behavioural patterns
  - **Frequent Subsequence Mining**
    - only frequent subsequences are of interest
    - many approaches & algorithms available
- **Suffix Trees**:
  - sequences from alphabet A & terminal symbol $
  - questions:
    - is q a suffix?
      - if path(q$) from root reaches a leaf
    - is q a substring?
      - if path(q) from root can be processed
    - how often does q occur?
      - follow path(q) from root & count leaves below
- **Trajectories**:
  - time series of spatial positions
  - resolution is often too high & redundant $\rightarrow$ expensive comparisons & high memory requirement
  - compress by omitting waypoints
    - **Douglas-Peucker Algorithm**: approximation error smaller than $\delta$
      - draw line from start to end, then break where error is largest until all errors are smaller than $\delta$
    - Compressing with Speed & Direction
      - waypoints where speed & direction is same can be omitted

### C. Encounter Detection

- finding out where a team succeeded/failed
- fights can happen when opponents are in fighting range (individual ranges must be considered)
- build graph from combat links & support links

### D. Ranking Player Skill

- controlling difficulty level and player progress
- important for match making & team creation
- skill level can be deduced from past victories & defeats
- **ELO System** (Arpad Elo, 1970):
  - a player's performance is normal distributed around his skill level
  - updating with zero-sum principle
  - needs at least 20 matches to stabilize
  - chronological order is important
  - no conclusions from team results
- **Team Skill**:
  - considers team chemistry
  - some players might be better combined with specific players
  - average over computed sub-team rankings
